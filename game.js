...
Game.buildMap = function(map) {
    ...
    for (var y = 0; y < map.length; y++) {
        ...
        for (var x = 0; x < map[y].length; x++) {
            for (var a = 0; a < Game.Tiles.length; a++) {
                if (Game.Tiles[a].index == map[y][x]) {
                    finishedMap[y][x] = Game.Tiles[a];
                } else {
                    if (map[y][x] >= Game.Tiles.length)
                        finishedMap[y][x] = Game.Tiles[0];
                }
            }
...
Game.compressMap = function(map) {
    var finishedMap = [];
    for (var y = 0; y < map.length; y++) {
        finishedMap.push([]);
        for (var x = 0; x < map[y].length; x++) {
            for (var a = 0; a < Game.Tiles.length; a++) {
                if (map[y][x].index ==  Game.Tiles[a].index) {
                    finishedMap[y][x] = Game.Tiles[a].index;
                } 
            }
